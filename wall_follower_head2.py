...
#my_path =  "SRRSLSRLLS"#"SRLSLRSSRSSRSRLSLRRLLRRL"#path
...
try:
	...
	def reset_callback(channel):
    	print("Reseting")				#Restart the Pi code
    	os.execv(__file__, sys.argv)

    def map_again_callback(channel):	
    	print("Mapping again")
    	subprocess.call(["python","wall_follower_head2.py"])	


    GPIO.add_event_detect(25,GPIO.FALLING,callback=map_again_callback)	
    GPIO.add_event_detect(8,GPIO.FALLING,callback=reset_callback)
	...
	def spot_right():
		...
		if start < (-180 + right_turn_angle):
			...
			while (start-output) < right_turn_angle:
				...
				output=float(ser.readline())
				print(output)
		#start_pos = start_pos - right_turn_angle
		#if start_pos < -180:
		#	start_pos = 360+start_pos
		#elif start_pos >180:
		#	start_pos = start_pos - 360 	
	...
	def spot_left():
		...
		if start > (180 -left_turn_angle):
			...
			while (output - start) < left_turn_angle:
				...
				output=float(ser.readline())
				print(output)
		#start_pos = start_pos + left_turn_angle	
		#if start_pos < -180:
		#	start_pos = 360+start_pos
		#elif start_pos >180:
		#	start_pos = start_pos - 360	
	...
	def final_pid(wall,front_select,check):
		global wall_pid,distance_new_cell
		sensor_array = mes_sense()

		if front_select == centre_thresh:
			...
			if wall == "both":
				...
				while (sensor_array[0]< left_thresh and sensor_array[2]>right_thresh):
					...
					if sensor_array[1] < front_select :
				...
				while (sensor_array[1]> front_select):
	...
	while True:
		...
		if my_path[i] == 'S':
			...
			final_pid(available_walls,centre_thresh,1)
			'''

			available_walls = check_available_walls()

			print ("Available walls for delay are:" + available_walls)

			final_pid_delay(available_walls)
			...
			final_pid(available_walls,centre_thresh,0)
